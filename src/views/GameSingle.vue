                    ...
                    <li v-for="vote in rating.votes" :key="vote._id">
                      <div class="columns is-marginless" style="align-items: center">
                        <div class="column is-9 is-paddingless">
                          <strong :class="getRatingValueClass(vote.value)">{{ vote.value }} </strong> <small><router-link :to="{ name: 'profile.show', params: { id: vote.user._id }}" class="has-text-grey-dark">{{ vote.user.first_name }} {{ vote.user.last_name | firstInitial }}</router-link></small>
                        </div>
